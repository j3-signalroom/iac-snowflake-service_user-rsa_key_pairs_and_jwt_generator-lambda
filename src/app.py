...
def lambda_handler(event, context):
    ...
    try:
        ...
        snowflake_service_user = event.get("snowflake_service_user", "").upper()
    ...
    logger.info("Snowflake Service User: %s", snowflake_service_user)
    logger.info("Secrets Path: %s", secrets_path)

    try:
        # Generate key pairs.
        key_pairs = GenerateKeyPairs(snowflake_account_identifier, snowflake_service_user, secrets_path)
